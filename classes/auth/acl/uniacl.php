...
class Auth_Acl_UniAcl extends \Auth_Acl_Driver
{
	...
	{
		static::$_valid_roles = array_keys(\Config::get('uniauth.roles'));
	}
	...
	public function has_access($condition, Array $entity)
	{
		...
		{
			$roles = \Config::get('uniauth.roles', array());
			...
			foreach ($current_roles as $r_role)
			{
				...
				{
					if (is_bool($r_rights[$area]))
					{
						return $r_rights[$area];
					}
					else{
